  ...
  const renderContent = () => {
    switch (screen) {
        ...
        return (
          <div className="p-8 w-full flex flex-col items-center">
            ...
            <div className="mt-6 mb-8 text-center">
              ...
              <p className="text-slate-500 text-sm">Ingresa tus datos para empezar</p>
            </div>
      ...
      case 'transition':
        ...
        return (
          <div className="p-10 text-center flex flex-col items-center">
            <div className="text-xs font-bold text-blue-500 mb-2 tracking-[0.3em]">NIVEL</div>
            <h2 className="text-4xl font-black mb-6 text-slate-800 tracking-tighter uppercase">{labels[diff]}</h2>
            <p className="text-slate-500 mb-10 text-sm">Rondas 10 de {totalRounds}.</p>
            <button 
              onClick={() => {
                setScreen('playing');
                startTime.current = Date.now();
        ...
        return (
          <div className="w-full p-8 flex flex-col items-center justify-between min-h-[520px]">
            <div className="flex justify-between items-center w-full mb-8">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 px-3 py-1 rounded-full">RONDA {round}/30</span>
            ...
            <div className="grid grid-cols-2 gap-8 p-8 bg-slate-50 rounded-[3rem] border border-slate-100 shadow-inner w-full aspect-square max-w-[280px]">
            ...
            <div className="w-full grid grid-cols-2 gap-4 mt-12">
              <button 
                ...
                className="bg-blue-50 text-blue-600 border border-blue-100 py-5 rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-blue-100 transition-all active:scale-95 shadow-sm"
              >
                Iguales
              </button>
              <button 
                onClick={() => handleResponse(false)}
                className="bg-pink-50 text-pink-600 border border-pink-100 py-5 rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-pink-100 transition-all active:scale-95 shadow-sm"
              >
                Diferente
              </button>
            </div>
      ...
      case 'results':
        ...
        return (
          <div className="p-10 text-center flex flex-col items-center w-full">
            ...
            <h2 className="text-2xl font-bold mb-2 text-slate-800 tracking-tight">¡Excelente!</h2>
            <p className="text-slate-500 mb-8 text-sm">Has terminado el experimento.</p>
            <div className="w-full bg-slate-50 p-6 rounded-3xl mb-8 border border-slate-100">
              <span className="text-slate-400 text-[10px] font-black uppercase tracking-widest block mb-1">Aciertos</span>
              <span className="text-4xl font-bold text-slate-800 tracking-tighter">{correctCount}<span className="text-lg text-slate-300 ml-1">/ 30</span></span>
            </div>
            <button 
              onClick={() => setScreen('registration')}
              className="w-full bg-slate-900 text-white py-5 rounded-2xl font-bold shadow-lg hover:bg-slate-800 transition-all active:scale-95"
            >
              Nuevo Participante
            </button>
          </div>
